...
| Device catalog (wattages, synonyms, photos) | 🚧 in progress | 2026-08-30 · AC nameplate presets (5, MCA/MOCP) + 17 appliance wattage presets (es-SV synonyms, Art. 120 categories); provenance infra live: `verifiedAt`/`source` fields, WATTAGES.md procedure, shrinking `KNOWN_UNVERIFIED_*` lint, «por verificar» count on the panel; UI inputs now watts-first (W ≈ VA, breakdown stays VA); spec-sheet verification run + photos pending |
